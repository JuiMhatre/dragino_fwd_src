...
int sx1302_set_antenna_params(int32_t amp, int32_t phi,uint8_t rf_chain_num) {
    ...
    if (rf_chain_num >= LGW_RF_CHAIN_NB) {
        DEBUG_MSG("ERROR: invalid RF chain\n");
        return LGW_REG_ERROR;
    }
    reg_amp = REG_SELECT(rf_chain_num,  SX1302_REG_RADIO_FE_IQ_COMP_AMP_COEFF_RADIO_A_AMP_COEFF,
                                SX1302_REG_RADIO_FE_IQ_COMP_AMP_COEFF_RADIO_B_AMP_COEFF);
    reg_phi = REG_SELECT(rf_chain_num, SX1302_REG_RADIO_FE_IQ_COMP_PHI_COEFF_RADIO_A_PHI_COEFF,
                                    SX1302_REG_RADIO_FE_IQ_COMP_PHI_COEFF_RADIO_B_PHI_COEFF);
    err_amp = lgw_reg_w(reg_amp, amp);
    err_phi = lgw_reg_w(reg_phi, phi);
